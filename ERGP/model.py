...
r"""
    ...
    Date: 2022/06/27
    Description: Efficient and scalable Recommendation via item-item Graph Partitioning (ERGP)
"""
import torch
import cupy as cp
import numpy as np
...
class ERGP(GeneralRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        # Recursive paritioning and linear autoencoder learning in partition 1
        self.update_S(np.arange(self.n_items)[np.where(first_split)]) 
        # Recursive paritioning and linear autoencoder learning in partition 2  
        self.update_S(np.arange(self.n_items)[np.where(~first_split)])
    ...
    def update_S(self, item_list):
        ...
        Update S (recursive)
        """
        if item_list.shape[0] < self.size_limit:
            # If the partition size is samller than size limit, learn the linear autoencoder for this partition.
            comm_inter = self.inter_mat[:, item_list]
            comm_inter = comm_inter.T @ comm_inter
            comm_ae = self.linear_autoencoder(
                comm_inter,
                self.V[item_list, :],
                self.d_i[item_list, :],
    ...
    def linear_autoencoder(self, inter_mat, V, d_i, d_i_inv) -> sp.coo_matrix:
        r"""
        Optimization of linear autoencoder in each partition
        """
        # Initialize
